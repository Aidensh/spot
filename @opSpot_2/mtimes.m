...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if isa(B,'SeisDataContainer') && isa(A,'opSpot_2')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
elseif ~isa(A,'opSpot_2') % A is not spot
    if isscalar(A) && (B.m ~= 1)
       % s*C (mode 3)
       if isa(A,'oppSpot_2') || isa(B,'oppSpot_2')
           y = oppFoG(A,B);
       else
           y = opFoG_2(A,B);
       end
...
%               a call to the "usual" matrix-vector product.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
elseif ~isa(B,'opSpot_2') % A is spot, B isnt
    if A.activated
        p = size(B,1);
        ...
        error('Inactivated operator is not supported for multiplication with regular MATLAB array')
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
