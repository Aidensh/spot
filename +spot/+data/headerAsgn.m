...
h.size(index) = [];
h.size   = [h.size(1:index(1)-1) size h.size(index(1)+1:end)];
h.origin(index) = [];
h.origin = [h.origin(1:index(1)-1) header.origin h.origin(index(1)+1:end)];
h.delta(index) = [];
h.delta  = [h.delta(1:index(1)-1) header.delta h.delta(index(1)+1:end)];
h.unit(index) = [];
h.unit   = [h.unit(1:index(1)-1) header.unit h.unit(index(1)+1:end)];
h.label(index) = [];
h.label  = [h.label(1:index(1)-1) header.label h.label(index(1)+1:end)];
